...
\acknowledgements{Sample Acknowledgements}
...
\begin{figure}[t]
\centering{
\includegraphics[width=0.5 \columnwidth]{Fig3a.eps}
...
%
% approximation of scattering from localized electrons, i.e.
% $S(q,\omega) = F_M(q)^2 S(\omega)$, with $S(\omega) =
% \frac{1}{2}\frac{1}{1-e^{-\beta\hbar\omega}}$Im$\chi(\omega)$.
% 
...
% and mechanical properties ranging from brittle $\alpha$-Pu to ductile $\delta$-Pu (2). The radius of Pu atoms in the face centered cubic $\delta$-phase is midway between that of Np and Am.
...
\bibliography{LaCoO3,Qualifier_Main,cRPA,Cerium}
